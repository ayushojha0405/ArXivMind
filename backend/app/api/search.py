...
def run_record_search(token: str, q: str, db: Session):
    try:
...
def search_papers(
    ...
    # Record search in the background so it doesn't delay the response
    if authorization and authorization.startswith("Bearer "):
        token = authorization.split(" ")[1]
        background_tasks.add_task(run_record_search, token, q, db)
